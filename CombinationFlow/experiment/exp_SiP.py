...
def joint_opt_SiP(dir_log, csys, placement, topo_graph, rtp, weight, tile_size, T_th, pid):
    ...
    def cost_func(csys, placement, stage, nv_dict):
        ...
        PPP = GIA.SiP(dir_dsent=dir_dsent,
        ...
        if PPP is None:
            ...
            pcost = PPP["pcost"]
            if stage == "init_joint":
                cvi = (1, {"thermal": therm.max(), "thermal_map": therm, **PPP})
                return cvi
                ...
                ci = {"thermal": therm.max(), "thermal_map": therm, **PPP}
                return (cv, ci)
            else:
                assert False

    placer = GIA.PlacerSA(csys=csys, cost_func=cost_func, cfg_algo={"init_n_j": 4, "T_end_j": 0.25}, pid=pid, dir_log=dir_log)
    placer.jopt(placement=placement)
    return placer.best_sol


def run_SiP(min_max_cpl, weight, T_th):
    """
        run SISL experiment
    ...
    for idx_d, d in enumerate(dt):
        ...
        dir_log_root = os.path.join(dir_data, "SiP/log_{}_{}".format(*min_max_cpl))
        dir_log = os.path.join(dir_log_root, "{}".format(idx_d))
        os.mkdir(dir_log)
        for pid in range(1):
            res.append(
                pool.apply_async(joint_opt_SiP,
                                 args=(dir_log, csys, placement, d["topo_graph"], d["rtp"], weight, d["tile_size"], T_th, pid)))
    pool.close()
    pool.join()
    for r in res:
        r.get()
...
if __name__ == "__main__":
    ...
    # run_SiP(min_max_cpl=(6, 10), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    # run_SiP(min_max_cpl=(11, 15), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    # run_SiP(min_max_cpl=(16, 20), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    run_SiP(min_max_cpl=(0, 0), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
