...
def joint_opt_PassiveGIA(dir_log, csys, placement, topo_graph, rtp, avg_lat_packet, weight, tile_size, T_th, pid):
    ...
    def cost_func(csys, placement, stage, nv_dict):
        ...
        PPP = GIA.PassiveGIA(dir_dsent=dir_dsent,
                             csys=csys,
                             placement=placement,
...
def run_PassiveGIA(min_max_cpl, weight, T_th):
    """
        run SISL experiment
    ...
    for idx_d, d in enumerate(dt):
        ...
        dir_log_root = os.path.join(dir_data, "PassiveGIA/log_{}_{}".format(*min_max_cpl))
        dir_log = os.path.join(dir_log_root, "{}".format(idx_d))
        os.mkdir(dir_log)
        for pid in range(2):
            res.append(
                pool.apply_async(joint_opt_PassiveGIA,
...
if __name__ == "__main__":
    ...
    # run_PassiveGIA(min_max_cpl=(6, 10), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    # run_PassiveGIA(min_max_cpl=(11, 15), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    # run_PassiveGIA(min_max_cpl=(16, 20), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)
    run_PassiveGIA(min_max_cpl=(0, 0), weight={"power_eu": 0.5, "perf": 0.5, "thermal": 0.1, "pcost": 0}, T_th=85)