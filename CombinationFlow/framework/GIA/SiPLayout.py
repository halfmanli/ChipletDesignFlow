...
def is_covered_cpl(csys, placement, x, y):
    """
        whether (x, y) covered by chiplet
    """
    for idx_c, (cx, cy, angle) in enumerate(placement):
        if cx <= x <= cx + csys.chiplets[idx_c].w(angle) and cy <= y <= cy + csys.chiplets[idx_c].h(angle):
            return True
    return False
...
def check_tile(csys, intp_graph, topo_graph, covered_zone, xy_rtr, xy_pin, x_t, y_t, rtr, cr):
    """
        ...
        rtr: the router need to be mapped
        cr: check radius
    """
    ...
    if (cnted_pin is not None) or not any(x_bl <= x_t < x_ur and y_bl <= y_t < y_ur
                                          for (x_bl, y_bl, x_ur, y_ur) in covered_zone):
        ic = sum([e_attr["nc"]
...
def map_router(csys, topo_graph, intp_graph, covered_zone, xy_rtr, xy_pin, rtr):
    """
...
def find_path(intp_graph, sd, xy_rtr, base_cost, bend_cost, y_cost, max_retry):
    """
    ...
    for (u, v, cha_uv), (_, w, cha_vw), e_attr in lg.edges(data=True):
        ...
        # add y penalty
        if u[0] != "vs" and w[0] != "vd":
            dir_uv = get_DIR(u, v)
            if (dir_uv == Direction.N or dir_uv == Direction.S):
                e_attr["b"] += y_cost
    ...
    for _ in range(max_retry):
        ...
        for se, de in zip(vs_edges, vd_edges):  # for every net
            ...
            path = nx.astar_path(G=lg, source=(*se, 0), target=(*de, 0), heuristic=heur,
...
def gen_layout_sip(csys, topo_graph, rtp, placement, faulted_links=None, base_cost=1, bend_cost=0, y_cost=0, max_retry=90):
    """
    ...
    covered_zone = [(cx, cy, cx + csys.chiplets[idx_c].w(angle), cy + csys.chiplets[idx_c].h(angle))
                    for idx_c, (cx, cy, angle) in enumerate(placement)]  # (x_bl, y_bl, x_ur, y_ur)

    intp_graph = nx.DiGraph()
    for (x_u, x_v) in zip(range(0, csys.W + 1), range(1, csys.W + 1)):  # add one column implicitly
        for y in range(csys.H):
            intp_graph.add_edge((x_u, y), (x_v, y), nc=1)  # only own normal channel
            intp_graph.add_edge((x_v, y), (x_u, y), nc=1)

    for (y_u, y_v) in zip(range(0, csys.H), range(1, csys.H)):
        for x in range(csys.W + 1):
            if (x == csys.W) or (not any(x_bl <= x < x_ur and y_bl <= y_u < y_ur
                                                            for (x_bl, y_bl, x_ur, y_ur) in covered_zone)
                                                    and not any(x_bl <= x < x_ur and y_bl <= y_v < y_ur
                                                                for (x_bl, y_bl, x_ur, y_ur) in covered_zone)):  # original is (x == csys.W or x == csys.W + 1)
                # for (x, y_u) not covered by chiplet and (x, y_v) not covered by chiplet, can put bridge chiplet to connect them
                # we must save (csys.W, 0) ~ (csys.W, csys.H - 1) empty to put bridge chiplet
                intp_graph.add_edge((x, y_u), (x, y_v), nc=1)
                intp_graph.add_edge((x, y_v), (x, y_u), nc=1)
