...
def find_path(intp_graph, sd, xy_rtr, base_cost, bend_cost, y_cost, max_retry):
    ...
    for (u, v, cha_uv), (_, w, cha_vw), e_attr in lg.edges(data=True):
        if u[0] == "vs":  # erase base cost for virtual source edges; can NOT set "vd" to 0, which is weight of the predecessor edge
            e_attr["b"] = 0
    ...
    for _ in range(max_retry):
        ...
        # equal to 0.5 in the first iteration and to 1.5 or 2 times its previous value in subsequent interations
        # definition of one iteration: After a single iteration of the algorithm, all nets are routed once as if they were the only connection to be routed
        p_fac *= 1.5
...
def gen_layout_passive(csys, topo_graph, rtp, placement, faulted_links=None, base_cost=1, bend_cost=1, y_cost=0, max_retry=90):
    """
