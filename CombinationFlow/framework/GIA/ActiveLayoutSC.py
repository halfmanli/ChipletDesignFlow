    ...
    for f_e in ig.edges():  # from_edge node
        for t_e in ig.edges(f_e[1]):  # to_edge node
            ...
            if f_e in vs_edges:
                ...
                if attr_f_e["nc"]:
                    lg.add_edge((*f_e, 0), (*t_e, 0))
            elif attr_f_e["nc"] and attr_t_e["nc"]:
                lg.add_edge((*f_e, 0), (*t_e, 0))
...
def gen_layout_active_sc(csys, topo_graph, rtp, placement, faulted_links=None, base_cost=1, bend_cost=0, max_retry=90):
    """
        Manhattan Layout Generation for active interposer but with one channel
    ...
    for (x_u, x_v) in zip(range(0, csys.W), range(1, csys.W)):
        for y in range(csys.H):
            intp_graph.add_edge((x_u, y), (x_v, y), nc=1)  # nc: normal channel
    ...
    for p in csys.task_graph:
        ...
        if csys.task_graph.in_degree(p) and intp_graph.has_edge(
            ...
            assert intp_graph.edges[e_So]["nc"] == 1
            intp_graph.edges[e_So]["nc"] -= 1
        if csys.task_graph.out_degree(p) and intp_graph.has_edge(*e_Si):
            assert intp_graph.edges[e_Si]["nc"] == 1
            intp_graph.edges[e_Si]["nc"] -= 1
